# -*- coding: utf-8 -*-

from odoo import fields, models, api, _
from odoo.exceptions import UserError
from odoo.tools import amount_to_text_en
import logging
_logger = logging.getLogger(__name__)


class FundtransferVoucher(models.Model):
    _name = "fund.transfer"

    name = fields.Char(string='Name', size=64, readonly=True)
    type = fields.Selection([
        ('in', 'Inward'),
       ('out', 'Outward'),
    ], string='Transfer Type', default='in', required=True)
    journal_id = fields.Many2one('account.journal', string="Journal", domain=[('type', '=', 'bank')], required=True)
    transaction_date = fields.Date(string='Transaction Date', required=True, default=fields.Date.context_today)
    partner_id = fields.Many2one('res.partner', string="Transfer from/to", required=True)
    commercial_partner_id = fields.Many2one('res.partner', string='Commercial Entity', compute_sudo=True,
                                            related='partner_id.commercial_partner_id', store=True, readonly=True,
                                            help="The commercial entity that will be used on Journal Entries for this invoice")
    account_id = fields.Many2one('account.account', string="Account", required=True)

    description = fields.Char(string='Description')
    amount = fields.Float('Amount', required=True)
    account_move_id = fields.Many2one('account.move', string="Accounting Entry", readonly=True)
    move_id = fields.Many2one('account.move', string='Journal Entry',
                              readonly=True, index=True, ondelete='restrict', copy=False,
                              help="Link to the automatically generated Journal Items.")
    state = fields.Selection([
        ('draft', 'Draft'),
       ('post', 'Posted'),
       ('cancel', 'Cancelled'),
    ], string='Status', default='draft', readonly=True)
    _sql_constraints = [('name_uniq', 'unique (name)', 'Fund Transfer must be unique !')]
    _defaults = {'name': lambda self, cr, uid, context: 'Fund Transfer'}

    @api.multi
    def unlink(self):
        for voucher in self:
            if voucher.state == 'post':
                raise UserError(_('You can not delete posted vouchers.'))
        return super(FundtransferVoucher, self).unlink()

    @api.multi
    def print_cash(self):
        return {
            'type': 'ir.actions.report.xml',
            'report_name': 'advanced_accounting.fund_transfer_report'
        }

    @api.multi
    def amount_to_text(self, amount, currency='AED'):
        convert_amount_in_words = amount_to_text_en.amount_to_text(amount, lang='en', currency='')
        convert_amount_in_words = convert_amount_in_words.replace(' and Zero Cent', ' Only ')
        convert_amount_in_words = convert_amount_in_words.replace('Cents', 'Fils')
        return convert_amount_in_words

    @api.onchange('partner_id')
    def _onchange_partner(self):
        if self.type == 'in':
            if self.partner_id.customer:
                self.account_id = self.partner_id.property_account_receivable_id
            elif self.partner_id.supplier:
                self.account_id = self.partner_id.property_account_payable_id
        elif self.type == 'out':
            if self.partner_id.supplier:
                self.account_id = self.partner_id.property_account_payable_id
            elif self.partner_id.customer:
                self.account_id = self.partner_id.property_account_receivable_id
        else:
            pass

    @api.onchange('type')
    def _onchange_type(self):
        if self.type == 'in':
            if self.partner_id.customer:
                self.account_id = self.partner_id.property_account_receivable_id
            elif self.partner_id.supplier:
                self.account_id = self.partner_id.property_account_payable_id
        elif self.type == 'out':
            if self.partner_id.supplier:
                self.account_id = self.partner_id.property_account_payable_id
            elif self.partner_id.customer:
                self.account_id = self.partner_id.property_account_receivable_id
        else:
            pass


    @api.multi
    def action_cancel(self):
        if self.env['res.users'].has_group('account.group_account_manager'):
            account_entry = self.account_move_id.id
            self.write({'account_move_id': False})
            journal_entry = self.env['account.move'].search([('id', '=', account_entry)])
            if len(journal_entry):
                journal_entry.button_cancel()        
                journal_entry.unlink()
                _logger.warning('=============================================== (%s).',journal_entry)
                self.write({'state': 'cancel'})
        else:
            raise UserError(_('You can not cancel the entry,to delete this entry user should belong to the Advisor group'))
             
    @api.multi
    def post(self):
        voucher_name = self.env['ir.sequence'].next_by_code('fund.transfer') or 'new'
        if self.journal_id.type != 'bank':
            raise UserError(_('Selected journal is not a Bank Journal.'))
        description = ""
        if self.description:
            description = self.description
        else:
            description='Fund Transfer'

        if self.type == 'in':

            line_ids = [
                (0, 0,
                {'journal_id': self.journal_id.id, 'account_id': self.account_id.id,
                    'name': description, 'partner_id': self.commercial_partner_id.id,
                    'amount_currency': 0.0, 'credit': self.amount}),
                    (0, 0, {'journal_id': self.journal_id.id, 'account_id': self.journal_id.default_debit_account_id.id,
                    'name': description, 'amount_currency': 0.0, 'debit': self.amount,
                    })
            ]
        else:
            line_ids = [
                (0, 0,
                 {'journal_id': self.journal_id.id, 'account_id': self.account_id.id,
                  'name': description, 'partner_id': self.commercial_partner_id.id,
                  'amount_currency': 0.0, 'debit': self.amount}),
                (0, 0, {'journal_id': self.journal_id.id, 'account_id': self.journal_id.default_credit_account_id.id,
                        'name': description, 'amount_currency': 0.0, 'credit': self.amount,
                        })
            ]

        vals = {
            'journal_id': self.journal_id.id,
            'ref': voucher_name,
            'narration': description,
            'date': self.transaction_date,
            'line_ids': line_ids,
        }
        account_move = self.env['account.move'].create(vals)
        account_move.post()
        self.write({'state': 'post', 'name': voucher_name, 'account_move_id': account_move.id})

    @api.multi
    def register_payment(self, writeoff_acc_id=False, writeoff_journal_id=False):
        if self.move_id:
            line_to_reconcile=[]
            """ Reconcile payable/receivable lines from the invoice with payment_line """
            for inv in self:
                line_to_reconcile += self.move_id.line_ids.filtered(
                lambda r: not r.reconciled and r.account_id.internal_type in ('payable', 'receivable'))
            line_to_reconcile += self.account_move_id.line_ids.filtered(
            lambda r: not r.reconciled and r.account_id.internal_type in ('payable', 'receivable'))
            _logger.warning('===============================================')
        return (line_to_reconcile).reconcile(writeoff_acc_id, writeoff_journal_id)

