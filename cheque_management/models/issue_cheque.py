...
class IssueCheque(models.Model):
    ...
    def print_cheque(self):
        ...
        # @chlpn: before running this if loop make sure you use the correct bank id, as of now we are giving this static
        if self.cheque_id.bank_name.id == 1:
        ...
        elif self.cheque_id.bank_name.id == 2:
            self._cr.execute('UPDATE ir_act_report_xml ' \
                             'SET paperformat_id=%s ' \
                             'WHERE name=%s', (pid, 'Cheque Print1',))
            return {
                'type': 'ir.actions.report.xml',
                'report_name': 'cheque_management.cheque_print_report1'
            }
        else:
            return {
                'type': 'ir.actions.report.xml',
                'report_name': 'cheque_management.cheque_print_report2'
            }
