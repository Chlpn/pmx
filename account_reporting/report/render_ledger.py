...
#        print'----',moveline_datas
        self.env.cr.execute("""select round(sum(debit)-sum(credit),2) as opening_balance from account_move_line as a left join account_move as b on a.move_id=b.id where a.account_id=%s and  b.state='posted' and a.date<%s""",(ledger_data.account_id.id,datetime.datetime.strptime(ledger_data.date_from, '%Y-%m-%d'),))
